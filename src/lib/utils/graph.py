...
def single_agent_states_to_multiagent_state(agent_H_state, agent_A_state):
    return '{0}_H^{1}_A'.format(agent_H_state, agent_A_state)

def single_agent_actions_to_multiagent_action(agent_H_action, agent_A_action):
    return single_agent_states_to_multiagent_state(agent_H_action, agent_A_action)
...
def get_unique_single_agent_actions_from_joint_actions(joint_actions):
    ...
    return [
        sorted(set([action_pair[0] for action_pair in action_pairs])), # Agent H actions
        sorted(set([action_pair[1] for action_pair in action_pairs])) # Agent A actions
    ]
...
def are_gridworld_states_multiagent(state_list):
    return all([
        bool(re.fullmatch(r'\(\d+, \d+\)_H\^\(\d+, \d+\)_A', state)) for state in state_list
    ])

def are_general_graph_states_multiagent(state_list):
    return all([
        bool(re.fullmatch(r'.*_H\^.*_A', state)) for state in state_list
    ])
...
def graph_to_joint_transition_tensor(mdp_graph, return_sparse=False):
    if is_graph_stochastic(mdp_graph):
        ...
        action_list_H, action_list_A = get_single_agent_actions_from_joint_mdp_graph(mdp_graph)
        transition_tensor_ = torch.zeros(len(state_list), len(action_list_H), len(action_list_A), len(state_list))

        for i in range(len(state_list)):
            for j in range(len(action_list_H)):
                for k in range(len(action_list_A)):
                    for l in range(len(state_list)):

                        try:
                            transition_tensor_[i][j][k][l] = mdp_graph[
                                build_stochastic_graph_node(
                                    single_agent_actions_to_multiagent_action(action_list_H[j], action_list_A[k]),
                                    state_list[i]
                                )
                            ][
                                build_stochastic_graph_node(
...
# First index is current state s; second index is Agent H action a_H; third index is Agent A action a_A; fourth index is next state s'.
...
def compute_full_transition_tensor(
    ...
    # Canonical ordering of dimensions is (current state, Agent H action, Agent A action, next state).
    # If policy_tensor corresponds to the policy for Agent A (and therefore the currently acting agent
    # is H, i.e., acting_agent_is_H == True), transpose axis 1 (Agent H action) and axis 2 (Agent A action)
    # before proceeding.
...
def graphs_to_full_transition_tensor(joint_mdp_graph, policy_graph, acting_agent_is_H=True, return_sparse=False):
    return compute_full_transition_tensor(
        ...
        acting_agent_is_H=acting_agent_is_H,
        return_sparse=return_sparse
    )

def any_graphs_to_full_transition_tensor(*transition_graphs, acting_agent_is_H=True, return_sparse=False):
    if len(transition_graphs) == 1: # Single agent case
        ...
        return graphs_to_full_transition_tensor(
            *transition_graphs, acting_agent_is_H=acting_agent_is_H, return_sparse=return_sparse
        )
