...
def save_graphs_and_generate_data(
    ...
    for input_graph_name in input_graphs_dict.keys():
        data.save_graph_to_dot_file(
            input_graphs_dict[input_graph_name], '{0}-{1}'.format(input_graph_name, run_name), folder=save_folder
        )

    return [{
        'graph_type': input_graph_name,
        'graph_name': graph_descriptions[input_graph_name],
