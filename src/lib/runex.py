...
def run_multiagent_with_reward_experiment(
    ...
    seed_policy_tensor_B = graph.graph_to_policy_tensor(transition_graphs[1], return_sparse=False) # The fixed policy tensor we give Agent B, that Agent H initially optimizes against

    print()
    print('Computing Agent H policies:')
    print()

    full_transition_tensor_H = graph.compute_full_transition_tensor( # Full Agent H transition tensor assuming Agent B seed policy
        joint_transition_tensor, seed_policy_tensor_B, acting_agent_is_H=True, return_sparse=True
    )
    ...
    all_policy_tensors_B = torch.stack([
        learn.compute_optimal_policy_tensor(
            optimal_values_B, full_transition_tensor_B_chunk[0]
        ) for optimal_values_B, full_transition_tensor_B_chunk in zip(
            all_optimal_values_B, misc.chunk_1d_tensor_into_list(all_full_transition_tensors_B, len(all_optimal_values_B))
        )
    ])

    all_full_transition_tensors_H = torch.stack([
        graph.compute_full_transition_tensor(
            joint_transition_tensor, policy_tensor_B, acting_agent_is_H=True, return_sparse=True
    ...
    return (
        ...
        {} if not diagnostic_mode else {
            ...
            'all_values_H_against_B': all_values_H,
...
def run_one_experiment(
    ...
    if sweep_type == 'single_agent':
        ...
        (
            reward_samples_H, reward_samples_B, power_samples_H, power_samples_B, diagnostic_dict
