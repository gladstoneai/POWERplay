...
def single_agent_to_multiagent_graph_node(
    ...
    states_and_actions_single = utils.decompose_stochastic_graph_node(current_agent_graph_node)
    action_index = 0 if len(states_and_actions_single) == 2 else 1

    return utils.build_stochastic_graph_node(*[
...
def create_multiagent_transition_graph(single_agent_graph, current_agent_is_A=True):
    ...
    for other_agent_state in utils.get_states_from_graph(single_agent_graph):
